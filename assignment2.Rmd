...
We first derive the likelihood function.

\begin{equation}
  \begin{split}
    L(\beta_0, \beta_1|\boldsymbol{x,y}) &= \prod^{n}_{i=1} \bigg([p_i(\beta_0,\beta_1)]^{y_i}[1-p(\beta_0,\beta_1)]^{1-y_i}\bigg)\\
    \implies \log L(\beta_0,\beta_1|\boldsymbol{x,y}) &= \prod^{n}_{i=1}\bigg( \frac{e^{\beta_0+x_i\beta_1}}{1+e^{\beta_0+x_i\beta_1}}\bigg)^{y_i} \bigg(\frac{1}{1+e^{\beta_0+x_i\beta_1}}\bigg)^{1-y_i} \\
    \implies \log L(\beta_0,\beta_1|\boldsymbol{x,y}) &= 
  \end{split}
\end{equation}
