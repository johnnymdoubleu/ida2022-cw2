...
cat("The value for beta0:", beta[1], "\n",
    "The value for beta1:", beta[2])
```
Using two different methods, `maxLik()` and `optim()`, we can see that we both obtained similar results with a small($0.001$) difference. Thus, we conclude that the MLE of $\beta=(\beta_0,\beta_1)$ is $(0.976, -2.48)$
