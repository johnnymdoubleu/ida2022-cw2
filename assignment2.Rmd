...
\begin{equation}
  \begin{split}
    ...
    \frac{\partial}{\partial \mu} Q(\boldsymbol{\theta}|\boldsymbol{\theta^{(t)}})&=0 \implies \mu^{(t+1)} =\frac{\sum^{n}_{i=1}\tilde{p}_i^{(t)}y_i}{\sum^{n}_{i=1}\tilde{p}_i^{(t)}} \\
    \frac{\partial}{\partial \sigma^2} Q(\boldsymbol{\theta}|\boldsymbol{\theta^{(t)}})&=0 \implies (\sigma^2)^{(t+1)}=\frac{\sum^{n}_{i=1}\tilde{p}_i^{(t)}y_i(y_i-\mu^{(t)})^2}{\sum^{n}_{i=1}\tilde{p}_i^{(t)}y_i}\\
...
$$|p^{(t+1)}-p^{(t)}|+|\mu^{(t+1)-\mu{(t)}}|+|(\sigma^2)^{(t+1)}-(\sigma^2)^{(t)}|+|\lambda^{(t+1)}-\lambda^{(t)}|<\varepsilon$$
