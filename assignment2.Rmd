...
\begin{equation}
  \begin{split}
    ...
    &=\sum^{m}_{i=1}\bigg(y_i(\beta_0+x_i\beta_i)\bigg)-\sum^{n}_{i=1}\bigg(\log(1 + e^{\beta_0 + x_i\beta_1})\bigg) + \sum^{n}_{i=m+1}\bigg( (\beta_0 + x_i\beta_1)p_i(\beta) \bigg) \\
...
\begin{equation}
  \begin{split}
    ...
    \implies \log L(\theta;\boldsymbol{y,z})&=\sum^{n}_{i=1}z_i\bigg(p\cdot\frac{1}{y_i\sqrt{2\pi\sigma^2}}\text{exp}\left\{\frac{1}{2\sigma^2}(\log y_i - \mu)^2\right\}\bigg)+\sum^{n}_{i=1}(1-z_i)\bigg((1-p)\cdot\lambda e^{-\lambda y_i}\bigg)
  \end{split}
\end{equation}
