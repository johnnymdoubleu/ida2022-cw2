...
cat("The value for beta0:", beta[1], "\n",
    "The value for beta1:", beta[2])
```
Using two different methods, `maxLik()` and `optim()`, with $\varepsilon=1e-6$ we can see that we both obtained similar results with a small($0.001$) difference. Thus, we conclude that the MLE of $\beta=(\beta_0,\beta_1)$ is $(0.976, -2.48)$
...
#Performing the EM algorithm on Mixture Model
...
hist(dataex5, breaks=45, main = "Mixture Model of Log-Normal & exp", 
     ...
     cex.main = 1.5, cex.lab = 1.5, cex.axis = 1.4, freq = F)
